...
# Called from: nadi:natural_disasters/acid_rain/create, nadi:natural_disasters/acid_rain/tick
# Datapck by 2mal3

# remove blocks
execute as @a[gamemode=!spectator,tag=!global.ignore] at @s run function nadi:natural_disasters/acid_rain/summon
...
execute as @e[tag=nadi.acid_rain,type=minecraft:area_effect_cloud] at @s unless block ~ ~-1 ~ #nadi:acid_rain/acid_rain_immune run setblock ~ ~-1 ~ minecraft:air
...
# timer
schedule function nadi:natural_disasters/acid_rain/tick 2s

scoreboard players remove $nadi.time nadi.data 1
execute if score $nadi.time nadi.data matches 0 run function nadi:natural_disasters/acid_rain/remove
