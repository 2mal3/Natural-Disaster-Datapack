...
# Called from: nadi:natural_disasters/blizzard/create, nadi:natural_disasters/blizzard/tick
# Datapack by 2mal3


# blizzard effects for all player not in rooms
tag @a[tag=nadi.blizzard_effect] remove nadi.blizzard_effect
execute as @a[gamemode=!spectator,tag=!global.ignore,tag=!global.ignore.gui] at @s if block ~ ~2 ~ minecraft:air if block ~ ~3 ~ minecraft:air if block ~ ~4 ~ minecraft:air if block ~ ~5 ~ minecraft:air if block ~ ~6 ~ minecraft:air run function nadi:natural_disasters/blizzard/effect
...
# timer
schedule function nadi:natural_disasters/blizzard/tick 1s

scoreboard players remove $nadi.time nadi.data 1
execute if score $nadi.time nadi.data matches 0 run function nadi:natural_disasters/blizzard/remove
