...
execute if score $nadi.notifications nadi.config matches 1 run tellraw @a {"text":"A acid rain begins.","color":"gray"}

# Loads the time for the acid rain from the config
scoreboard players operation $nadi.time nadi.data = $nadi.acid_rain.time nadi.config
