...
# Called from: 2mal3:nadi/natural_disasters/firestorm/create, 2mal3:nadi/natural_disasters/firestorm/tick
# Datapck by 2mal3

# summon fire
execute as @a[gamemode=!spectator,tag=!global.ignore] at @s run function 2mal3:nadi/natural_disasters/firestorm/summon
...
# timer
schedule function 2mal3:nadi/natural_disasters/firestorm/tick 2s

scoreboard players remove $nadi.time nadi.data 1
execute if score $nadi.time nadi.data matches 0 run function 2mal3:nadi/natural_disasters/firestorm/remove
