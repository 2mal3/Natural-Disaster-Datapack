...
execute if score $nadi.notifications nadi.config matches 1 run tellraw @a {"text":"A firestorm begins.","color":"gray"}

# Loads the time for the firestorm from the config
scoreboard players operation $nadi.time nadi.data = $nadi.firestorm.time nadi.config
