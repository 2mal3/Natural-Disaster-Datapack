...
# Called from: 2mal3:nadi/natural_disasters/sandstorm/create, 2mal3:nadi/natural_disasters/sandstorm/tick
# Datapack by 2mal3


# sandstorm effects
tag @a[tag=!nadi.sandstorm_effect] remove nadi.sandstorm_effect
execute as @a[gamemode=!spectator,tag=!global.ignore,tag=!global.ignore.gui] at @s if block ~ ~2 ~ minecraft:air if block ~ ~3 ~ minecraft:air if block ~ ~4 ~ minecraft:air if block ~ ~5 ~ minecraft:air if block ~ ~6 ~ minecraft:air run function 2mal3:nadi/natural_disasters/sandstorm/effect


# place sand
execute as @a[gamemode=!spectator,tag=!global.ignore] at @s run function 2mal3:nadi/natural_disasters/sandstorm/summon
...
# timer
schedule function 2mal3:nadi/natural_disasters/sandstorm/tick 1s
